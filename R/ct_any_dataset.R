...
#'   inconsistent.
#'
#' @return A tibble containing rows from the requested Connecticut Open Data
#'   endpoint.
#'
#' @details
#' `ct_any_dataset()` bypasses the package catalog and sends a request directly
#' to the supplied JSON endpoint. Unlike [ct_pull_dataset()], it does not look
...
#'
#' @export
ct_any_dataset <- function(json_link,
                             limit = 10000,
                             timeout_sec = 30,
                             clean_names = TRUE,
                             coerce_types = TRUE) {
